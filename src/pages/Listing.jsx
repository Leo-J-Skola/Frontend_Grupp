...
const navigate = useNavigate();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await createListing(title, rooms, pricePerNight, description, location, image, startDate, endDate);
      navigate("/");
    } catch (err) {
  ...
  return (
      ...
      <Form.Group className="mb-3" controlId="formBasicHostId">
      <Form.Label>Host</Form.Label>
      <Form.Control type="text" value={hostId} onChange={(e) => sethostId(e.target.value)}/>
      </Form.Group>
      ...
      <Form.Control type="date" placeholder="Enter Listing Start Date" value={startDate} onChange={(e) => setStartDate(e.target.value)}/>
      <Form.Label>Pick end date</Form.Label>
      <Form.Control type="date" placeholder="Enter Listing End Date" value={endDate} onChange={(e) => setEndDate(e.target.value)} />
      <Button variant="secondary" type="submit" className='listing-submit' >Create Listing</Button>
      </Form.Group>
      </Form>
